...
Onsets::Onsets(const DurationList& d)
{
    ...
    for (std::list<Rational>::const_iterator i = d.begin(); i != d.end(); ++i)
    {
        accu += *i;
        _content.push_back(accu);
    }
...
{
    DurationList res;
    std::list<Rational>::const_iterator i = _content.cbegin();
    assert (*i == Rational(0));
    Rational prev = *i;
...
const Onsets operator+(const Onsets& lhs, const Onsets& rhs)
{
    Onsets res;
    int mode = 0;
    std::list<Rational>::const_iterator i = lhs._content.cbegin();
    std::list<Rational>::const_iterator j = rhs._content.cbegin();
    assert (*i == Rational(0));
    assert (*j == Rational(0));
